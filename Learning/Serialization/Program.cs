...
using NuJsonSerializer = System.Text.Json.JsonSerializer;
...
namespace Serialization
{
    internal class Program
    {
        private static async Task Main(string[] args)
        {
            ...
            WriteLine(); // Display the serialized object graph
            ...
            using (FileStream jsonLoad = File.Open(jsonPath, FileMode.Open))
            {
                ...
                var loadedPeople = (List<Person>)
                    await NuJsonSerializer.DeserializeAsync(utf8Json: jsonLoad, returnType: typeof(List<Person>)); foreach (var item in loadedPeople)
                {
